        ...
        bindEvent: function() {
            ...
            function popupArea(e) {
                ...
                _self.gearArea.innerHTML = '<div class="area_ctrl slideInUp">' +
                    ...
                    '<div class="area_btn larea_cancel" style="text-align: left;">取消</div>' +
                    '<div class="area_btn larea_finish" style="text-align: right;">确定</div>' +
                    '</div>' +
                ...
                //获取gearArea并判断事件源
                var abc=document.querySelector(".gearArea");
                abc.addEventListener('touchstart', function(){
                    event = event ? event : window.event;
                    var bj = event.srcElement ? event.srcElement : event.target;
                    if(bj.className!="gearArea"){
                       return
                    }else{

                        $(".area_ctrl").animate({top:"+=57%"},500,function(){
                            _self.close(e)
                        });
